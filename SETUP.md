...
3. **性能の数字**（Phase 17 の実機テスト）: URL の最後に `?debug` を付けて開くと（例: `https://contour.xxx.workers.dev/?debug`）、左下に数字が出ます。
   ...
   - `NET` の最後の `fix` は、ルームに「そこまで行けたはずがない」と位置を戻された回数です。**普通に遊んでいて 0 のまま**なら正常です。
     増えたら、そのとき何をしていたか（走っていた、敵に飛ばされた、裂け目を通った、電波が悪かった など）を教えてください
...
GitHub Pages など Cloudflare 以外に置いたコピーや、ファイルを直接開いた場合は、**1人プレイ**になります（以前使っていた公開 MQTT ブローカーは、誰でも何でも送れてしまうため外しました）。
...
| 表世界の交通（2人以上のとき） | 車・歩行者をルームが動かし、1人あたり約 3 KB/s（1時間で約 10 MB）受信します。1人のときは今までどおりスマホが動かし、通信はありません |
| BEELZEBUB の記憶 | 全ルーム共通の Durable Object 1つ（`BossMind`）。公開時に自動で作られます（設定は不要）。呼ばれるのは、参加したとき・戦いの始まりと終わり・戦闘中 30 秒ごとだけ。学習の進み具合は `https://…workers.dev/mind` で見られます |
...
- **ONLINE にならない（SOLO のまま）**: URL が `workers.dev` のものか確認してください。それ以外の URL では1人プレイです。
  `workers.dev` でも SOLO なら、その日の無料枠を使い切った可能性があります（日本時間の朝9時に戻ります）。ゲームは1人で続き、30 秒ごとにつながり直そうとします。
